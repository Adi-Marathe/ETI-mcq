...
def parse_ete_unit5(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    content = clean_text(content)
    
    # Split by number followed by period (handling optional bolding and whitespace)
    parts = re.split(r'\n(?:\**)(\d+)\.\s*', '\n' + content)
    ...
    for i in range(1, len(parts), 2):
        ...
        explanation = f"The correct answer is {answer.split(') ')[1]} as per the Unit 5 syllabus on Tariff and Smart Billing."
...
questions = parse_ete_unit5('ete_unit5_new.txt')
with open('../src/data/ete_unit5.json', 'w', encoding='utf-8') as f:
    json.dump(questions, f, indent=4)

print(f"Successfully processed {len(questions)} questions for ETE Unit 5.")
