...
def parse_ete_new(filepath):
    ...
    for i in range(1, len(parts), 2):
        ...
        for opt_char in ['A', 'B', 'C', 'D']:
            # Match opt_char) followed by text until next opt or Answer:
        ...
        # Basic explanation generation (will be enhanced or kept simple)
        explanation = f"The correct answer is {answer.split(') ')[1]} as per the Unit 1 syllabus on Digitalization beyond Automation."
...
questions = parse_ete_new('ete_unit1_new.txt')
with open('../src/data/ete_unit1.json', 'w', encoding='utf-8') as f:
    json.dump(questions, f, indent=4)

print(f"Successfully processed {len(questions)} questions for ETE Unit 1.")
